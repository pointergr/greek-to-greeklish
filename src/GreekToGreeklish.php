...
class GreekToGreeklish
{
    ...
    /**
     * Ελέγχει τη λειτουργία της μετατροπής με συγκεκριμένες λέξεις και τις αναμενόμενες μετατροπές τους
     * 
     * @return array Πίνακας με τα αποτελέσματα
     */
    public function runTests() {
        // Πίνακας με ζευγάρια [ελληνική λέξη => αναμενόμενη τιμή greeklish]
        $testCases = array(
            'Ελληνικά' => 'Ellinika',
            'Άυλος' => 'Aylos',
        ...
        foreach ($testCases as $greek => $expectedGreeklish) {
            $actualGreeklish = $this->convert($greek);
            
            // Case-sensitive σύγκριση
            $passed = ($actualGreeklish === $expectedGreeklish);
            
            if ($passed) {
                $passedTests++;
            }
            
            // Προσθήκη διαφορών κατά χαρακτήρα για εύκολο εντοπισμό
            $diff = '';
            if (!$passed) {
                if (mb_strlen($expectedGreeklish, 'UTF-8') === mb_strlen($actualGreeklish, 'UTF-8')) {
                    for ($i = 0; $i < mb_strlen($expectedGreeklish, 'UTF-8'); $i++) {
                        $expChar = mb_substr($expectedGreeklish, $i, 1, 'UTF-8');
                        $actChar = mb_substr($actualGreeklish, $i, 1, 'UTF-8');
                        
                        if ($expChar !== $actChar) {
                            $diff .= "Θέση " . ($i+1) . ": '$expChar' αντί για '$actChar', ";
                        }
                    }
                    ...
                    if ($diff) {
                        ...
                        $diff = "Διαφορετική κωδικοποίηση χαρακτήρων";
                    }
                } else {
                    $diff = "Διαφορετικό μήκος: " . mb_strlen($expectedGreeklish, 'UTF-8') . " vs " . mb_strlen($actualGreeklish, 'UTF-8');
                }
            }
            
            $results[] = array(
                'original' => $greek,
                'expected' => $expectedGreeklish,
                'actual' => $actualGreeklish,
                'passed' => $passed,
                'diff' => $diff
            );
        }
